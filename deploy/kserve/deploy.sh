...
if [ "$SKIP_VALIDATION" = false ]; then
    ...
    detect_predictor_selector_label() {
        ...
        if oc get pods -n "$NAMESPACE" -l "serving.kserve.io/llm-inferenceservice=${name}" --no-headers &> /dev/null; then
            echo "serving.kserve.io/llm-inferenceservice"
            return
        fi

        if oc get pods -n "$NAMESPACE" -l "app.kubernetes.io/name=${name}" --no-headers &> /dev/null; then
            echo "app.kubernetes.io/name"
            return
        fi

        if oc get pods -n "$NAMESPACE" -l "serving.kserve.io/inferenceservice=${name}" --no-headers &> /dev/null; then
            echo "serving.kserve.io/inferenceservice"
            return
        fi

        if oc get pods -n "$NAMESPACE" -l "app=${name}" --no-headers &> /dev/null; then
            echo "app"
            return
        fi
...
echo ""
echo "2. Test model auto-routing (should pick Model-B):"
echo "   curl -k -X POST https://\$ENVOY_ROUTE/v1/chat/completions \\"
...
echo ""
echo "3. Test model auto-routing (should pick Model-A):"
echo "   curl -k -X POST https://\$ENVOY_ROUTE/v1/chat/completions \\"
echo "     -H \"Content-Type: application/json\" \\"
